...
ISR(PCINT1_vect)
{
	...
	if (old_state == 0) {
		...
		if (a==1) {new_state = 1; count++; pwmTimer1++;}
		if (b==1) {new_state = 2; count--; pwmTimer1--;}
		if (a==0 && b==0) new_state = 0;
	}
	else if (old_state == 1) {

	    // Handle A and B inputs for state 1
		if (a==0) {new_state = 0; count--; pwmTimer1--;}
		if (b==1) {new_state = 3; count++; pwmTimer1++;}
		if (a==1 && b==0) new_state = 1;
	}
	else if (old_state == 2) {

	    // Handle A and B inputs for state 2
		if (a==1) {new_state = 3; count--; pwmTimer1--;}
		if (b==0) {new_state = 0; count++; pwmTimer1++;}
		if (a==0 && b==1) new_state = 2;
	}
	else {   // old_state = 3

	    // Handle A and B inputs for state 3
		if (a==0) {new_state = 2; count++; pwmTimer1++;}
		if (b==0) {new_state = 1; count--; pwmTimer1--;}
...
void timer1_init()
{
	...
	TCCR1B |= (1 << WGM13) | (1 << WGM12) | (1 << CS12) | (1 << CS10); //Prescalar 1024
